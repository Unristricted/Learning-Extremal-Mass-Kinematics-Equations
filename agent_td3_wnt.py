    ...
    def run(
        ...
        while done < total_evals:
            ...
            if retrain_every > 0 and done % retrain_every == 0 and done < total_evals:
                ...
                model, x_scaler, y_scaler, n_train = train_reward_model(df2, target=self.target, device=self.device)
...
# Build & train policy
# -----------------------------
def build_and_train_policy(csv_path, target, device, param_lo, param_hi, seed, td3_timesteps):
...
def main():
    ...
    if not args.train_only:
        if td3 is None or env is None:
            # Load policy + rebuild env from current surrogate
            td3, env = build_and_train_policy(
                csv_path=args.csv,
                target=args.target,
                device=args.device,
                ...
                param_hi=args.param_hi,
                seed=args.seed,
                td3_timesteps=1,  # dummy; will be overwritten by load
            )
        ...
        # This is the part you asked for: after training, an agent is initialized and starts searching.
        agent.run(
        ...
        # Save updated policy if we retrained during search (or just to checkpoint)
        td3.save(args.policy_path)
